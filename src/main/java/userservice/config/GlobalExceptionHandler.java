...
public class GlobalExceptionHandler {
	...
	/**
	 * Handles validation failures triggered by @Valid or @Validated annotations
	 * in controller request bodies or parameters.
	 *
	 * <p>This method captures {@link MethodArgumentNotValidException} and extracts
	 * all field-level validation errors from the . Each error
	 * is formatted as:
	 * <pre>
	 *     fieldName: errorMessage
	 * </pre>
	 *
	 * <p>All field error messages are concatenated into a single comma-separated
	 ...
	 * @return ResponseEntity containing a standardized ApiResponse with
	 *         HTTP 400 (Bad Request) status and aggregated validation error message
	 */
	 ...
	 *
	 * <p>This method captures {@link BadCredentialsException} typically thrown
	 * during login attempts when the provided email/username or password is incorrect.
	 * For security reasons, it does not expose specific details about which field
	 * caused the failure and instead returns a generic error message.
	 *
	 * <p>This helps prevent user enumeration and protects sensitive authentication logic.
	 *
	 * <p>Example response:
	 * <pre>
	 * {
	 ...
	 * </pre>
	 *
	 * @param ex the exception thrown when authentication fails due to invalid credentials
	 * @return ResponseEntity containing a standardized ApiResponse with
	 *         HTTP 401 (Unauthorized) status and a generic authentication error message
	 */
	 ...
	 *
	 * <p>This method captures {@link ResourceNotFoundException}, which is typically
	 * thrown when an entity (e.g., User, Order, etc.) does not exist in the system
	 * for the given identifier or search criteria.
	 *
	 * <p>The exception message is returned directly in the response to provide
	 ...
	 * @return ResponseEntity containing a standardized ApiResponse with
	 *         HTTP 404 (Not Found) status and a descriptive error message
	 */
	 ...
	 *
	 * <p>This method acts as a global fallback handler for any exception that is not
	 * explicitly handled by more specific {@code @ExceptionHandler} methods.
	 * It ensures that the application does not expose internal implementation details
	 * or sensitive information to the client.
	 *
	 * <p>Instead of returning the actual exception message, a generic error message
	 * is sent in the response for security and consistency purposes.
	 *
	 * <p><b>Note:</b> The actual exception should be logged internally (e.g., using a logger)
	 * for debugging and monitoring, even though it is not exposed to the client.
	 *
	 * <p>Example response:
	 * <pre>
	 * {
	 ...
	 * @return ResponseEntity containing a standardized ApiResponse with
	 *         HTTP 500 (Internal Server Error) status and a generic error message
	 */
